...
export default function Login() {
  ...
  // Mengecek apakah sesi berakhir dari URL param
  useEffect(() => {
  ...
  // Logika Redirect setelah sukses login (Full Reload)
  useEffect(() => {
    if (!showSuccessModal || !redirectPath) return;

    const redirectTimer = window.setTimeout(() => {
      // Full reload wajib untuk Next.js 16 + Supabase SSR agar middleware mendeteksi cookie baru
      window.location.assign(redirectPath);
    }, 700);

    return () => window.clearTimeout(redirectTimer);
  }, [showSuccessModal, redirectPath]);

  // Mengecek status lockdown dari localStorage
  useEffect(() => {
    const checkLockout = () => {
      if (typeof window !== 'undefined') {
  ...
  // Timer hitung mundur untuk lockdown
  useEffect(() => {
    let timer;
  ...
  const handleNikChange = (e) => {
    // Memastikan hanya angka yang bisa masuk
  ...
  const handleLogin = async (e) => {
    // PENTING: Mencegah browser me-reload halaman dan menambah '?' di URL
    e.preventDefault();
    ...
    try {
      const response = await fetch('/api/auth/login', {
        ...
        // Hapus credentials: 'include' jika tidak menggunakan custom domain eksternal 
        // untuk mencegah konflik CORS di localhost.
      ...
      if (!response.ok) {
        if (response.status === 403) {
          setErrorMsg(data.error || 'Akun sedang diverifikasi.');
          setIsLoading(false);
          return;
        }
        handleFailedLogin(data.error || 'Login gagal. Periksa NIK dan password Anda.');
        return;
      }
      ...
      setRedirectPath(data.redirectTo || '/dashboard/warga');
