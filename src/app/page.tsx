...
// TAMBAHAN: Import komponen CardPenggalangan
import CardPenggalangan from '../components/CardPenggalangan';
...
export default function Home() {
  ...
  // TAMBAHAN: State untuk Penggalangan Dana
  const [listPenggalangan, setListPenggalangan] = useState<any[]>([]);
  const [isLoadingPenggalangan, setIsLoadingPenggalangan] = useState(true);
  ...
  // TAMBAHAN: useEffect untuk mengambil data Penggalangan Dana
  useEffect(() => {
    const fetchPenggalanganDana = async () => {
      setIsLoadingPenggalangan(true);
      try {
        const { data, error } = await supabase
          ...
          .select(`
            *,
            kebutuhan_dana (*),
            donatur_penggalangan (*)
          `)
          .eq('status', 'Aktif') // Hanya menampilkan acara yang masih aktif
          .order('created_at', { ascending: false });
        
        if (error) throw error;
        if (data) {
          setListPenggalangan(data);
        }
      } catch (err: any) {
        console.error('Gagal mengambil data penggalangan dana:', err.message);
      } finally {
        setIsLoadingPenggalangan(false);
      }
    };

    fetchPenggalanganDana();
  }, [supabase]);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans selection:bg-blue-200 selection:text-blue-900">
      ...
      <main className="flex-1">
        ...
        {/* TAMBAHAN: 2.5 PENGGALANGAN DANA */}
        {(listPenggalangan.length > 0 || isLoadingPenggalangan) && (
          <section className="py-20 sm:py-28 bg-[#F8FAFC] border-t border-gray-200/50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex flex-col items-center text-center mb-14 animate-fade-in-up">
                <span className="w-12 h-1.5 bg-green-500 rounded-full mb-4"></span>
                <h2 className="text-3xl sm:text-4xl font-black text-gray-900 tracking-tight">Program Penggalangan Dana</h2>
                <p className="text-gray-500 mt-3 text-base sm:text-lg max-w-2xl">Mari berpartisipasi dan bantu wujudkan program serta kebutuhan lingkungan kita bersama.</p>
              </div>

              {isLoadingPenggalangan ? (
                <div className="flex flex-wrap justify-center gap-6">
                  {[1, 2, 3].map((n) => (
                    <div key={n} className="w-full sm:w-[350px] h-[450px] bg-white rounded-3xl animate-pulse shrink-0 border border-gray-100 shadow-sm">
                      <div className="h-48 bg-gray-200 rounded-t-3xl"></div>
                ...
                <div className="flex flex-wrap justify-center gap-6 sm:gap-8 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                  {listPenggalangan.map((acara: any) => (
                    <div key={acara.id} className="w-full sm:w-auto flex justify-center">
                      <CardPenggalangan acara={acara} />
                    </div>
                  ))}
                </div>
              )}
