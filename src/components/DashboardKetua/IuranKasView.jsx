...
export default function IuranKasView({
  ...
  const toPeriodIndex = (bulan, tahun) => {
    const idx = DAFTAR_BULAN.indexOf(bulan);
    if (idx === -1) return -1;
    return Number(tahun) * 12 + idx;
  };

  const cekStatusIuran = (nik) => {
    // Pengecekan histori kepatuhan: cek tunggakan bulan sebelumnya, bukan bulan yang sedang diproses
    ...
    const periodeSaatIni = toPeriodIndex(filterBulanIuran, filterTahunIuran);
    const bulanSudahBayar = new Set(
      pernahLunas
        .map(r => toPeriodIndex(r.bulan_iuran, r.tahun_iuran))
        .filter(i => i >= 0)
    );

    const minPaidIndex = Math.min(...bulanSudahBayar);
    let adaTunggakan = false;
    for (let i = minPaidIndex; i < periodeSaatIni; i++) {
      if (!bulanSudahBayar.has(i)) {
        adaTunggakan = true;
        break;
      }
    }
