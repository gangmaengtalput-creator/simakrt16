...
const DesilFields = ({ values, onChange }) => {
  ...
  return <div className="col-span-1 sm:col-span-2 bg-cyan-50 p-5 rounded-2xl border border-cyan-100"><h4 className="font-black text-cyan-800 mb-4">Data Desil Warga</h4><div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    {numberField('pendapatan_kerja', 'Pendapatan dari kerja / bulan')}{numberField('pendapatan_usaha', 'Pendapatan dari usaha / bulan')}{numberField('pendapatan_pemberian', 'Pendapatan pemberian orang lain / bulan')}{numberField('pengeluaran_makan_mingguan', 'Pengeluaran makan / minggu')}{numberField('pengeluaran_listrik_bulanan', 'Pengeluaran listrik / bulan')}{numberField('pengeluaran_lain_bulanan', 'Pengeluaran selain makan / bulan')}{numberField('pengeluaran_tahunan', 'Pengeluaran tahunan')}
    <div className="bg-white p-3 rounded-xl"><span className="text-xs font-bold text-gray-500 block">Total pendapatan / bulan</span><strong className="text-lg text-green-700">Rp {totalPendapatan.toLocaleString('id-ID')}</strong></div><div className="bg-white p-3 rounded-xl"><span className="text-xs font-bold text-gray-500 block">Total pengeluaran / bulan</span><strong className="text-lg text-red-700">Rp {Math.round(totalPengeluaran).toLocaleString('id-ID')}</strong></div><div className="sm:col-span-2 bg-white p-3 rounded-xl"><span className="text-xs font-bold text-gray-500 block">Tabungan setiap bulan</span><strong className={totalPendapatan > totalPengeluaran ? 'text-green-700' : 'text-red-700'}>{totalPendapatan > totalPengeluaran ? 'Ya' : 'Tidak'}</strong></div>
  </div></div>;
};

const CustomFields = ({ definitions, values, onChange }) => definitions.length > 0 ? <div className="col-span-1 sm:col-span-2 bg-violet-50 p-5 rounded-2xl border border-violet-100"><h4 className="font-black text-violet-800 mb-4">Field Custom</h4><div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{definitions.map(field => <div key={field.id}><label className="font-bold text-gray-700 block mb-1.5">{field.label} <span className="text-xs text-gray-400">({field.target})</span></label>{field.field_type === 'boolean' ? <select value={values?.[field.field_key] ?? ''} onChange={e => onChange(field.field_key, e.target.value === 'true')} className="w-full border border-gray-300 p-2.5 rounded-xl bg-white"><option value="">Pilih...</option><option value="true">Ya</option><option value="false">Tidak</option></select> : <input type={field.field_type} value={values?.[field.field_key] ?? ''} onChange={e => onChange(field.field_key, field.field_type === 'number' ? Number(e.target.value) : e.target.value)} className="w-full border border-gray-300 p-2.5 rounded-xl bg-white" />}</div>)}</div></div> : null;
...
export default function DataWargaView({ setActiveView, dataWarga, fetchWarga }) {
  ...
  useEffect(() => {
    supabase.from('custom_field_definitions').select('*').order('created_at', { ascending: true }).then(({ data }) => setCustomFieldDefinitions(data || []));
  }, [supabase]);
            ...
            <div className="p-6 overflow-y-auto bg-gray-50 flex-1">
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm">
                  ...
                  <div><label className="font-bold text-gray-700 block mb-1.5">Tanggal masuk warga</label><input type="date" name="tanggal_masuk" value={formData?.tanggal_masuk || ''} onChange={handleInputChange} className="w-full border border-gray-300 p-2.5 rounded-xl bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-all" /></div>
            ...
            <div className="p-6 overflow-y-auto bg-gray-50 flex-1">
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm">
                  ...
                  <div><label className="font-bold text-gray-700 block mb-1.5">Tanggal masuk warga</label><input type="date" name="tanggal_masuk" value={formData?.tanggal_masuk || ''} onChange={handleInputChange} className="w-full border border-gray-300 p-2.5 rounded-xl bg-gray-50 focus:bg-white focus:ring-2 focus:ring-amber-500 outline-none transition-all" /></div>
